<?php
	...
	if (!$proceed) { 
		echo "Form processing halted for suspicious activity <br />";
	}
	
	/* Has too much time elapsed? If so, we take the customer to the error page. */
	if (((int)$_POST['mms'] + $seconds) < mktime()) {
		echo "We're sorry, there was a problem processing your request. Your session has expired. <br />";
	}
	...
	if (!preg_match('/^[_A-z0-9-]+((\.|\+)[_A-z0-9-]+)*@[A-z0-9-]+(\.[A-z0-9-]+)*(\.[A-z]{2,4})$/', $tafEmail)) {
		echo "Are You A Bot?!?!?!?!?! <br />";
	}
	
	/* Set TO Email Addresses (Uncomment if using multiple addresses) */
	...
	mail($tafFriendEmail, $tafName." found a product at ".$storeName." that you might like", $message, $headers);
	...
	if (mail) {
		...
		echo "We're sorry, there was a problem processing your request. <br />";
		exit();
	}
?>
